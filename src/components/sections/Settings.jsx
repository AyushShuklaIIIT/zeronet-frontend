...
const Settings = ({ user }) => {
    ...
    const saveSchoolDetails = (e) => {
        e.preventDefault();
        alert(isAdmin ? 'School details saved!' : 'Profile updated!');
    };
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 fade-in'>
            <div className='lg:col-span-2 space-y-6'>
                ...
                {isAdmin ? (
                    ...
                    <div className='bg-slate-800 rounded-2xl p-6 border border-slate-700'>
                        <h3 className='text-lg font-semibold text-white mb-4'>My Profile Details</h3>
                        <form onSubmit={saveSchoolDetails}>
                            <div className='space-y-4'>
                                <div>
                                    <label className='block text-sm font-medium text-slate-300 mb-2'>Full Name</label>
                                    <input type="text" defaultValue={user?.name || 'Teacher User'} className='w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-blue-500' />
                                </div>
                                <div>
                                    <label className='block text-sm font-medium text-slate-300 mb-2'>Email Account (Locked)</label>
                                    <input type="email" defaultValue={user?.email || 'teacher@iiitb.ac.in'} disabled className='w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-xl text-slate-400 cursor-not-allowed focus:outline-none' />
                                </div>
                                <div className='mt-4'>
                                    <button type="submit" className='px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl transition'>Update Profile</button>
                                </div>
                            </div>
                        </form>
                    </div>
